...
def MAJ_L_voisins(dico_cube, cube, figure):
	...
	#print(dico_cube[cube].num)
	#dico_cube[cube].L_voisins=o.L_voisins_init #<- ca marche pas mais je sais pas pourquoi...
	#print(dico_cube[cube].L_voisins)
	dico_cube[cube].L_voisins=[0 for i in range(6)]
	#print(o.L_voisins_init==[0 for i in range(6)])
	...
	#print(" ")
	return dico_cube[cube].L_voisins
...
def placement_possible(position, cube, figure, dico_cube):
	...
	"""
	dico_cube[cube].L_voisins=MAJ_L_voisins(dico_cube, cube, figure)
	ok = False
	i = 0
	#print(dico_cube[cube].L_voisins)
	if (dico_cube[cube].L_voisins==[0 for i in range(6)]) :
		ok=True
	else : 
		while not(ok) and i< len(dico_cube[cube].L_voisins) :
			#print(i, ok)
			if dico_cube[cube].L_voisins[i] != 0 and not(ok):
				#print(dico_cube[cube].num, dico_cube[i].num)
		...
		if placement_possible(position, cube, figure, dico_cube) and not gf.present(cube, figure):
			if x <len(figure) and y <len(figure[0]) and z <len(figure[0][0]) and x >=0 and y >=0 and z >=0 :
				...
				for i in dico_cube[cube].L_voisins :
					#print(i)
					if i != 0 :
						#dico_cube[i].L_voisins=[]
						dico_cube[i].L_voisins=MAJ_L_voisins(dico_cube, i, figure) #pas tres optimal...
	return place
